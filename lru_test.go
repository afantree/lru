	...
	for i := 0; i < b.N; i++ {
		for _, tt := range getTests {
			...
			if ok != tt.expectedOk {
				...
				b.Fatalf("%s expected get to return 1234 but got %v", tt.name, val)
			}
		}
	}
}
...
func BenchmarkRemove(b *testing.B) {
	for i := 0; i < b.N; i++ {
		...
		lru.Add("myKey", 1234)
		if val, ok := lru.Get("myKey"); !ok {
			b.Fatal("TestRemove returned no match")
		} else if val != 1234 {
			b.Fatalf("TestRemove failed.  Expected %d, got %v", 1234, val)
		}

		lru.Remove("myKey")
		if _, ok := lru.Get("myKey"); ok {
			b.Fatal("TestRemove returned a removed entry")
		}
	}
}
